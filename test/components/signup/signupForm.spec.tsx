...
describe('singup Form:', () => {
  ...
  test('SignupForm to be rendered with needed element', () => {
    ...
    const inputIdentificationElement = signUpComponent.getByPlaceholderText(
      'Identification Number provided by government'
    );
    expect(inputIdentificationElement).toHaveAttribute('type', 'text');
    expect(inputIdentificationElement).toHaveValue('');
  ...
  test('SignupForm submits value of form on submit', async () => {
    ...
    await user.type(signUpComponent.getByPlaceholderText('Identification Number provided by government'), 'Nepal@123');
